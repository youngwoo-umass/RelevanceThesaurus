...
def delete_compute_pep(deleter, pep_client_request, tokenizer, pair_data):
    ...
    c_log.info("Request %d items", len(candidate_list))
    scores_np = pep_client_request(candidate_list)
    c_log.info("Done")
    save_item_per_qd: list[tuple[BothSegPartitionedPair, np.array]] = pairzip(candidate_list, scores_np)
    ...
    columns = ["delete", 'remain', 'score1', 'score2']
    table = [columns]
    indices_list = []
    for i, (bsp_pair, scores) in enumerate(save_item_per_qd):
        seg2: IndicesPartitionedSegment = bsp_pair.segment2
        info = {
            'delete': len(seg2.del_indices1),
            'remain': len(seg2.tokens) - len(seg2.del_indices1),
            'score1': two_digit_float(scores[0]),
            'score2': two_digit_float(scores[1]),
        }
        indices_list.append(seg2.del_indices1)
        row = [info[c] for c in columns]
        table.append(row)
...
def main():
    ...
    for pair_data, _attn in itr:
        save_item_per_qd = delete_compute_pep(deleter, pep_client_request, tokenizer, pair_data)
        save_items.append(save_item_per_qd)
        if len(save_items) > save_batch_size:
            save_path = path_join(save_dir, f"{part_no}_{batch_idx}")
            pickle.dump(save_item_per_qd, open(save_path, "wb"))
            batch_idx += 1
