...
def main():
    ...
    itr = tsv_iter(term_pair_scores_path)
    # itr = [(bert_normalize(q_term), bert_normalize(d_term), score) for q_term, d_term, score in itr]
    ...
    for q_term, entries in grouped.items():
        ...
        n_promising = 0
        for _q_term, d_term, score_s in entries:
            if float(score_s) > t_promising:
                try:
                    n_promising += 1
